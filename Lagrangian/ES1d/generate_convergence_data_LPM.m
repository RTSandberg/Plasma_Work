...
% collect for v = 1.5
analyticsv1p5fixt = zeros(2,numNx);
analyticsv1p5fixx = zeros(2,numdelt);
for ii = 1:2 
...
% for v = -.2
analyticsvnp2fixt = zeros(3,numNx);
analyticsvnp2fixx = zeros(3,numdelt);
for ii = 1:3 
    L = Llist(ii);
    for jj = 1:numNx
        Nx = Nxlist(jj);
        make_input_deck(run_day, run_name, input_deck,xmin,L,Nx,delv,vmin,vmax)
        save_movie = 0;
        plot_in_run = 0;
        delt = .01;
        tf = 50;
